...
class SECONDNET(nn.Module):
    def __init__(self, model_cfg):
        ...
        out_channels = self.model_cfg.downsample_layers[-1]['filters'][-1]
...
class DBiGNN(nn.Module):
    def __init__(self, model_cfg):
        super(DBiGNN, self).__init__()
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        # self.use_voxel_feature = use_voxel_feature
        self.model_cfg = model_cfg
        self.conv_input = spconv.SparseSequential(
...
class BiGNN_Light(nn.Module):
    def __init__(self, model_cfg):
        super(BiGNN_Light, self).__init__()
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        # self.use_voxel_feature = use_voxel_feature
        self.model_cfg = model_cfg
        self.conv_input = spconv.SparseSequential(
        ...
        self.voxels_fps_downsample_fns = []
        for i in range(len(self.model_cfg.fpsdownsamplers)):
            voxel_downsample_fn = partial(fps_voxel_downscale_operation, **self.model_cfg.fpsdownsamplers[i])
            self.voxels_fps_downsample_fns.append(voxel_downsample_fn)
    ...
    def forward(self, x, **kwargs):
        ...
        for i, layer in enumerate(self.downsample_layers):
            ...
            if i == 0:
                sparserx, raw_sizes = self.voxels_fps_downsample_fns[i](x, batch_size=kwargs['batch_size'])
            else:
                sparserx = self.voxels_fps_downsample_fns[i](x, batch_size=kwargs['batch_size'], raw_sizes=raw_sizes)
            
            rx_list.append(sparserx)
...
class BiGNN_Light2(nn.Module):
    def __init__(self, model_cfg):
        super(BiGNN_Light2, self).__init__()
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        # self.use_voxel_feature = use_voxel_feature
        self.model_cfg = model_cfg
        self.conv_input = spconv.SparseSequential(
        ...
        self.voxels_upsample_fn = partial(threeNN_sparser_to_densor, **self.model_cfg.threeNNupsampler)
        # for i 

        last_pad = (0, 0, 0)
        
        out_channels = self.model_cfg.downsample_layers[-1]['filters'][-1] + 64 * len(self.model_cfg.groupers) + 64
    ...
    def forward(self, x, **kwargs):
        ...
        lrx = torch.cat([rx_list[-1].features, *lrx_list], dim=-1)
        rx_list[-1].features = lrx
        interpolated_feats = self.voxels_upsample_fn(x, rx_list[-1], batch_size=kwargs['batch_size'])
        # interpolated_feats = interpolated_feats
        x.features = torch.cat([x.features, interpolated_feats], dim=-1)
        # [200, 176, 5]
        out = self.conv4_out(x)
