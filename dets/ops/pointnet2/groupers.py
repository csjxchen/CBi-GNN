...
class GrouperDisAttention_reproduce_v1(nn.Module):
    def __init__(self, 
                radius, 
                nsamples, 
                mlps, 
                ...
                instance_norm=False, 
                pool_methods=['max_pool']):
        super(GrouperDisAttention_reproduce_v1, self).__init__()
        self.xyz_mlp_spec = xyz_mlp_spec
        mlp_spec = mlps
        self.pool_methods = pool_methods
        self.radius = radius
        self.nsamples = nsamples
        ...
        # xyz_mlp_spec = xyz_mlps[i]
        self.mlps = pt_utils.SharedMLP(mlp_spec, bn=False, instance_norm=instance_norm)
        self.xyz_mlps = pt_utils.SharedMLP(self.xyz_mlp_spec, bn=False, instance_norm=instance_norm)
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None):
        '''
            xyz: B, N, 3
            new_features: B, C, N
        ...
        return new_xyz, new_features
class GrouperDisAttention_reproduce_v2(nn.Module):
    def __init__(self, 
                radius, 
                nsamples, 
                mlps, 
                ...
                instance_norm=False, 
                pool_methods=['max_pool']):
        super(GrouperDisAttention_reproduce_v2, self).__init__()
        self.xyz_mlp_spec = xyz_mlp_spec
        mlp_spec = mlps
        self.radius = radius
        self.nsamples = nsamples
        ...
        # mlp_spec = mlps[i]
        # xyz_mlp_spec = xyz_mlps[i]
        self.mlps = pt_utils.SharedMLP(mlp_spec, bn=feat_mlp_bn, instance_norm=instance_norm)
        self.xyz_mlps = pt_utils.SharedMLP(self.xyz_mlp_spec, bn=xyz_mlp_bn, instance_norm=instance_norm)
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None):
        '''
            xyz: B, N, 3
            new_features: B, C, N
