...
int ball_query_wrapper_fast_repeat(int b, int n, int m, float radius, int nsample, 
    ...
    const float *xyz = xyz_tensor.data<float>();
    int *idx = idx_tensor.data<int>();
    int *idn = idn_tensor.data<int>();
    
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    ball_query_kernel_launcher_fast_repeat(b, n, m, radius, nsample, new_xyz, xyz, idx, idn, stream);
    return 1;
}
...
int grid_query_wrapper_fast(int b, int n, int m, float radius, int nsample, 
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    grid_query_kernel_launcher_fast(b, n, m, radius, nsample, new_xyz, xyz, idx, idn, stream);
    return 1;
}
