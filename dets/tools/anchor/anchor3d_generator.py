...
def create_anchors_3d_stride(feature_size, 
    ...
    x_stride, y_stride, z_stride = anchor_strides
    x_offset, y_offset, z_offset = anchor_offsets
    x_centers = np.arrange(feature_size[2], dtype=dtype)
    y_centers = np.arrange(feature_size[1], dtype=dtype)
    z_centers = np.arrange(feature_size[0], dtype=dtype)
    ...
    anchor_size = np.array(anchor_size, dtype=dtype)
    anchor_size = np.reshape(anchor_size, (-1, 3))
    tile_shape = [1] * 5
    for i in range(len(rets)):
        rets[i] = np.tile(rets[i][..., np.newaxis, :], anchor_size.shape[0])
        rets[i] = rets[i][..., np.newaxis]
    
    anchor_size = np.reshape(anchor_size, [1, 1, 1, -1, 1, 3])
    # x_len, y_len, z_len, size_len, rotation_len, size_data_len
    tile_size_shape = list(rets[0].shape)
    tile_size_shape[3] = 1
    anchor_size = np.tile(anchor_size, tile_size_shape)
